...
internal static class ConfigExtensions {
    ...
    public static bool IsPressedEx(this ConfigEntry<KeyboardShortcut> configEntry) {
        KeyboardShortcut value = configEntry.Value;
        if (value.MainKey == KeyCode.None) {
            return false;
        }

        bool mainKey = UnityInput.Current.GetKey(value.MainKey);
        bool modifiers = value.Modifiers.All(UnityInput.Current.GetKey);
