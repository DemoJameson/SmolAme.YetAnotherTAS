...
public class Program : WindowsFormsApplicationBase {
    ...
    protected override bool OnStartup(StartupEventArgs eventArgs) {
        ...
        // if IsSingleInstance = true and game launch before studio the connection will fail, idnw...
        // so we just close the studio already launched
        if (!IsSingleInstance) {
            try {
                foreach (Process process in Process.GetProcessesByName("TAS.Studio")) {
                ...
                MessageBox.Show("TAS.Studio does not support multiple instances, please close other instances manually.",
