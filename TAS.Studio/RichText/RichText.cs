...
public class RichText : UserControl {
    ...
    private void TryMoveCursorBehindFrame() {
        Place start = Selection.Start;
        if (Selection.IsEmpty && InputFrame.InputFrameRegex.IsMatch(Lines[start.iLine])) {
    ...
    private void TweakFrames(bool up, MouseEventArgs e = null) {
        ...
        InputFrame startFrame = Studio.Instance.InputRecords[startLine];
        InputFrame endFrame = Studio.Instance.InputRecords[endLine];

        if (!startFrame.IsInput && !endFrame.IsInput) {
            return;
        }

        if (!startFrame.IsInput) {
            startFrame = endFrame;
        } else if (!endFrame.IsInput) {
            endFrame = startFrame;
        }
        ...
        if (startFrame == endFrame) {
            if (up) {
                startFrame.Frames++;
            } else if (startFrame.Frames > 0) {
                startFrame.Frames--;
            }
        } else {
            if (up) {
                if (endFrame.Frames <= 0) {
                    return;
                }
